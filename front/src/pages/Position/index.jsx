...
function Index() {
    ...
    const actionDelete = async (position_ID) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                axios
                    .post(endpoint, { position_ID, position_StatusID: 0 })
                    .then(() => {
                        Swal.fire(
                            'Eliminado!',
    ...
    const actionEdit = async (position_ID) => {
        ...
        const response = await axios.get(
            `${endpoint}?position_ID=${position_ID}`
        );
    ...
    const handleOnClickSearch = async (page) => {
        ...
        const endpoint = `${env.apiURL}listPosition`;
        const response = await axios.get(
            `${endpoint}?page=${page_}&per_page=${perPage}&position_Name=${position_Name}&position_CreationDate=${position_CreationDate}`
        );
        setData(response.data.data);
        setPage_(response.data.page);
        setTotalRows(response.data.total);
