...
function Index() {
    ...
    const actionVerify = async (user_ID) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                axios
                    .post(endpoint, { user_ID, user_ApprovedStatus: 1 })
                    .then(() => {
                        fetchUsers(page_);
                    })
                    .catch(() => {
                ...
                const endpoint = `${env.apiURL}verifyUser`;
                axios
                    .post(endpoint, { user_ID, user_ApprovedStatus: 0 })
                    .then(() => {
                        fetchUsers(page_);
                    })
                    .catch(() => {
    ...
    const actionDelete = async (user_ID) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                axios
                    .post(endpoint, { user_ID, user_StatusID: 0 })
                    .then(() => {
                        fetchUsers(page_);
                    })
                    .catch(() => {
    ...
    const handleOnClickSearch = async () => {
        ...
        const endpoint = `${env.apiURL}list`;
        const response = await axios.get(
            `${endpoint}?page=${page_}&per_page=${perPage}&user_Name=${user_Name}&user_CreationDate=${user_CreationDate}&user_ApprovedStatus=${user_ApprovedStatus}`
        );
        setData(response.data.data);
        setPage_(response.data.page);
        setTotalRows(response.data.total);
