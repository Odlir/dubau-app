...
function Index() {
    ...
    const actionDelete = async (role_ID) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                axios
                    .post(endpoint, { role_ID, role_StatusID: 0 })
                    .then(() => {
                        Swal.fire(
                            'Eliminado!',
    ...
    const handleOnClickRegister = async (e) => {
        ...
        await axios
            .post(endpoint, { role_Name, role_Description, role_StatusID: '1' })
            .then(() => {
    ...
    const handleOnClickUpdate = async (e) => {
        ...
        await axios
            .post(endpoint, { role_ID: dataxRole, role_Name, role_Description })
            .then(() => {
                window.location.reload();
            })
            .catch(() => {
    ...
    const handleOnClickSearch = async (page) => {
        ...
        const endpoint = `${env.apiURL}listRole`;
        const response = await axios.get(
            `${endpoint}?page=${page_}&per_page=${perPage}&role_Name=${role_Name}&role_CreationDate=${role_CreationDate}`
        );
        setData(response.data.data);
        setPage_(response.data.page);
        setTotalRows(response.data.total);
