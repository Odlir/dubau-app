...
function Index() {
    ...
    const [commercial_section_Description, setCommercialSection_Description] =
        useState('');
    const [DescriptionAgain, setDescriptionAgain] = useState('');
    const [commercial_section_CreationDate, setCommercialSectionCreationDate] =
        useState('');
    const [
        commercial_section_ApprovedStatus,
    ...
    const actionDelete = async (commercial_section_ID) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                axios
                    .post(endpoint, {
                        commercial_section_ID,
                        commercial_section_StatusID: 0,
                    })
                    .then(() => {
                        Swal.fire(
                            'Eliminado!',
    ...
    const actionEdit = async (commercial_section_ID) => {
        ...
        const response = await axios.get(
            `${endpoint}?commercial_section_ID=${commercial_section_ID}`
        );
        ...
        setCommercialSection_Description(
            response.data.commercial_section_Description
        );
        setFormType('edit');
    };
    ...
    const handleOnClickRegister = async (e) => {
        ...
        await axios
            .post(endpoint, {
                commercial_section_Name,
                commercial_section_Description,
                commercial_section_StatusID: '1',
            })
            .then(() => {
    ...
    const handleOnClickUpdate = async (e) => {
        ...
        await axios
            .post(endpoint, {
                commercial_section_ID: dataxCommercialSection,
                commercial_section_Name,
                commercial_section_Description,
            })
            .then(() => {
    ...
    const handleOnClickSearch = async () => {
        ...
        const endpoint = `${env.apiURL}listCommercialSection`;
        const response = await axios.get(
            `${endpoint}?page=${page_}&per_page=${perPage}&commercial_section_Name=${commercial_section_Name}&commercial_section_CreationDate=${commercial_section_CreationDate}`
        );
        setData(response.data.data);
        setPage_(response.data.page);
        setTotalRows(response.data.total);
